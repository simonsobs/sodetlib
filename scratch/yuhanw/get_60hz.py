...
#example: get_60hz.py -data /data/smurf_data/20210219/1613770613/outputs/xxxx.dat -out /xxxx/xxxx/xxx -plot true
...
for c, (b, ch) in enumerate(zip(bands, channels)):
    ...
    try:
        ...
        freq_mask = ((np.abs(f) < target_freq + 1) & (np.abs(f) > target_freq - 1))
...
plt.figure(figsize=(12,8))
plt.suptitle('60Hz pick up')
plt.subplot(2, 1, 1)
plt.hist(target_noise_freq_list, color = 'g', edgecolor='gray', alpha=0.3,range = (59,61), bins=100);
...
plt.tight_layout()
plt.savefig('{}/{}_60_Hz_summary.png'.format(output,ctime),bbox_inches='tight', transparent=False,facecolor=(1,1,1,1))
