...
def recompute_psats(iva2, psat_level=0.9):
    ...
    iva2['p_sat_V1'] = np.full(iva2['p_sat'].shape, np.nan)
    
    for i in range(iva2['nchans']):
        if np.isnan(iva2['R_n_V1'][i]):
            continue
...
def recompute_si(iva2):
    ...
    """
    iva2['si_V1'] = np.full(iva2['si'].shape, np.nan)
    smooth_dist = 5
    ...
    for i in range(iva2['nchans']):
        ...
        if np.isnan(iva2['R_n_V1'][i]) or sc_idx == -1: 
            # iva2['si'][i] = np.nan
            continue

        # Running average
        i_tes_smooth = np.convolve(iva2['i_tes_V1'][i], w, mode='same')
        v_tes_smooth = np.convolve(iva2['v_tes_V1'][i], w, mode='same')
...
def reanalyze_iv(iv_file, psat_level=0.9, save=False, update_cfg=False, show_pb=False):
    """
    ...
    iva2 = np.load(iv_file, allow_pickle=True).item()

    R_sh = iva2['meta']['R_sh']
    R_bl = iva2['meta']['bias_line_resistance']

    iva2['i_bias_V1'] = iva2['v_bias'] / R_bl
    iva2['v_tes_V1'] = np.full(iva2['v_tes'].shape, np.nan)
    iva2['i_tes_V1'] = np.full(iva2['i_tes'].shape, np.nan)
    iva2['p_tes_V1'] = np.full(iva2['p_tes'].shape, np.nan)
    iva2['R_V1'] = np.full(iva2['R'].shape, np.nan)
    iva2['R_n_V1'] = np.full(iva2['R_n'].shape, np.nan)
    iva2['R_L_V1'] = np.full(iva2['R_L'].shape, np.nan)

    for i in trange(iva2['nchans'], disable=(not show_pb)):
        sc_idx = iva2['idxs'][i, 0]
        nb_idx = iva2['idxs'][i, 1]
        
        R = R_sh * (iva2['i_bias_V1']/(iva2['resp'][i]) - 1)
        ...
        iva2['v_tes_V1'][i] = iva2['v_thevenin'] * (R_tes / (R_tes + R_L))
        iva2['i_tes_V1'][i] = iva2['v_tes'][i] / R_tes
        iva2['p_tes_V1'][i] = iva2['v_tes'][i]**2 / R_tes
    ...
    if save:
        # fname = iv_file + 'with_RPcurve.npy'
        components = iv_file.split('/')
        name = components[-1][:-4]
        fname = name + 'with_RPcurve.npy'
        np.save(fname, iva2, allow_pickle=True)
...
def rerun_analysis(bs_file, iva2=None, save=False, R0_thresh=30e-3):
    ...
    **NOTE: User must run reanalyze_iv(IVA),
    ...
    """
    bsa2 = np.load(bs_file, allow_pickle=True).item()
    ...
    if save:
        # fname = iv_file + 'with_RPcurve.npy'
        components = bs_file.split('/')
        name = components[-1][:-4]
        fname = name + 'with_RPcurve.npy'
        np.save(fname, bsa2, allow_pickle=True)
...
def recompute_TES_params_from_RPcurve(bsa2, iva, rmin=0.1, rmax=0.99):
    """
    Computes I0_RP, R0_RP, Pj_RP, Si_RP from RP fitted RP curve.
    """
    n = int(np.ceil(max([1. / rmin, 1. / (1. - rmax), 1000])))
    n = min([n, 100000])
...
def recompute_dc_params(bsa2, iva2=None, R0_thresh=30e-3):
    ...
    I0, R0, Pj, Si = recompute_TES_params_from_RPcurve(bsa2, iva2)
...
def recompute_bsa_rfrac(bsa2, iva2):
    ...
    Rfrac_RP : array of (nchans)
        R0_RP / R_n_IV
    """
    bsa2['R_n_IV_RP'] = iva2['R_n_V1']
    bsa2['Rfrac_RP'] = bsa2['R0_RP'] / bsa2['R_n_IV_RP'] 