...
def check_channel_tickle(phases, max_I_span):
	pass
	...
	for c in channels:
		...
		if 1.1*amp_check > I_command:
			...
			popt,pcov = opt.curve_fit(f = sine_fit,xdata = time_plot, ydata = pnew,
								...
								pi,1.05*fguess]))
			rms_error = np.sqrt(np.sum((sine_fit(time_plot,*popt) - pnew)**2)/len(pnew))
		...
		if make_channel_plots:
			...
			if channel_data['detector_channel']:
				ax.plot(time_plot, sine_fit(time_plot, *popt),'r--',alpha = 0.8,linewidth = 3)
				...
				text_lines.append(f"R_guess = {channel_data['R_guess']/1e-3:.2f} mOhm")
			...
			ax.set_title(f'Band {band}, Channel {c}, Freq {np.round(channel_data["res_freq"],2)} MHz')
