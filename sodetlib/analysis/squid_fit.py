...
def plot_squid_curves(datafile, S=None, plot_dir=None):
    ...
    for band in bands:
        for i, ch in enumerate(dat[band]["channels"]):
            ...
            plt.figure()
            plt.plot(bias, 1e3 * (dat[band]["fvsfr"][i,:] - np.min(dat[band]["fvsfr"][i,:])))
...
def estimate_fit_parameters(
    phi,
    noisy_squid_curve,
    nharmonics_to_estimate=5,
    debug=False,
    ...
    yspan = ymax - ymin
    yoffset = yspan / 2.0 + ymin
    harmonic = lambda n, ph, phoff, amplitude: (amplitude) * np.cos(
        n * (ph - phoff) * (2.0 * np.pi / phi0)
    )
    first_harmonic_guess = lambda ph, phoff: harmonic(
        1, ph, phoff, (yspan / 2)
    ) + harmonic(0, ph, phoff, (yoffset))
    ...
    normalization_factor = (np.max(d_ms) - np.min(d_ms)) / (
        np.max(fit_guess) - np.min(fit_guess)
    )
