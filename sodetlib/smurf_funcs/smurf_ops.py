...
def uxm_setup(S, cfg, bands):
    """
    Tunes a UXM from scratch in the specified bands, 
    using the values specified in the DetConfig file
    ...
    bands : list or numpy.ndarray
        Iterable of bands you would like to tune
    
    Returns
    --------
    plotdir : str
        filepath to the plot directory on the 
        smurf-server where the relevant tuning plots
        are saved

    tunefile : str
        filepath to the tunefile written at the end of 
        the tuning, after cuts are made on bad tracking
        channels
    ...
    slot_num = <slot here>
    bands = <bands you want to tune>

    cfg = DetConfig()
    cfg.load_config_files(slot=slot_num)
    S = cfg.get_smurf_control()

    plotdir,tunefile = so.uxm_setup(S, cfg, bands)
    """
    ...
    S.take_noise_psd(20, nperseg=2**16, save_data=True, make_channel_plot=False,
    ...
    return S.plot_dir, S.tune_file

def uxm_relock(S,cfg,bands,input_tune_file):
    """
    Retunes a UXM from an associated tunefile, 
    using the values specified in the DetConfig file
    ...
    bands : list or numpy.ndarray
        Iterable of bands you would like to tune
    input_tune_file : str
        Full filepath to tunefile you would like
        to load the tune from
    ...
    slot_num = <slot here>
    bands = <bands you want to tune>
    input_tune_file = <tune file filepath to load>
