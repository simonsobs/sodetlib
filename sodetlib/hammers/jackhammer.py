...
def check_server_connection(server_port, retry=False, timeout=180, ping_timeout=1):
    ...
    # message to check if server is ready
        ...
        while True:
            ...
            if t_iter - start > timeout:
                ...
                raise TimeoutError(
                    ...
                    f"connection on port {server_port}"
                )
            if do_ping():
                break
            dt = time.time() - t_iter
    ...
    if dump_rogue_tree:
        ...
        for slot in slots:
            if check_server_connection(9000 + 3 * slot, retry=False):
                ...
                print(f"Could not connect to server for slot {slot}")
...
def hammer(slots=None, no_reboot=False, no_dump=False, skip_setup=False,
           dump_rogue=False, ping_timeout=120, server_timeout=180,
           setup_timeout=300):
    ...
    server_timeout : int
        Seconds to wait for each slot's server connection.
    setup_timeout : int
    ...
    for slot in active_slots[:]:
        try:
            ...
            msg = f"Server connection failed: {e}"
            cprint(f"Slot {slot}: {msg}", style=TermColors.FAIL)
            failed_slots[slot] = msg
            active_slots.remove(slot)
