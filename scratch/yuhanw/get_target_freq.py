...
#example: get_target_freq.py -data /data/smurf_data/20210219/1613770613/outputs/xxxx.dat -out /xxxx/xxxx/xxx -plot true -freq 1
...
for c, (b, ch) in enumerate(zip(bands, channels)):
    ...
    try:
        ...
        if chan_plot == 'true':
            ...
            plt.savefig('{}/{}_band_{}_chan_{}.png'.format(output,ctime,b,ch),bbox_inches='tight', transparent=False,facecolor=(1,1,1,1))
...
plt.figure(figsize=(12,8))
plt.suptitle('{}Hz pick up'.format(target_freq))
plt.subplot(4, 1, 1)
plt.hist(target_noise_freq_list, color = 'g', edgecolor='gray', alpha=0.3,range = (target_freq-0.2,target_freq+0.2), bins=100);
...
plt.hist(target_noise_level_list, color = 'b', edgecolor='gray',range = (med_level-500,med_level+500), alpha=0.3, bins=200);
...
plt.subplot(4, 1, 3)
log_noise_level = np.log10(target_noise_level_list)
med_log_level = np.median(log_noise_level)
plt.hist(log_noise_level, color = 'b', edgecolor='gray', alpha=0.3, bins=200);
# plt.axvline(med_log_level, label = r'$\mathrm{median\ amp\ }  $'+ f'{med_level:0.2f}' +
#                         r'$\mathrm{pA}/\sqrt{\mathrm{Hz}}$', color = 'r', linestyle='--')
plt.grid(linestyle = '--')
plt.legend(fontsize=14, loc=1)
plt.ylabel('$\mathrm{Channel\ count}$')
plt.xlabel('Amp [pA/rtHz] in log10 space')
...
log_noise_level = np.log10(target_noise_level_list)
med_log_level = np.median(log_noise_level)
plt.hist(log_noise_level, color = 'b', edgecolor='gray', range = (3,np.max(log_noise_level)), alpha=0.3, bins=200);
# plt.axvline(med_log_level, label = r'$\mathrm{median\ amp\ }  $'+ f'{med_level:0.2f}' +
#                         r'$\mathrm{pA}/\sqrt{\mathrm{Hz}}$', color = 'r', linestyle='--')
plt.grid(linestyle = '--')
plt.legend(fontsize=14, loc=1)
...
plt.tight_layout()
plt.savefig('{}/{}_{}_Hz_summary.png'.format(output,ctime,target_freq),bbox_inches='tight', transparent=False,facecolor=(1,1,1,1))
print(target_freq)
