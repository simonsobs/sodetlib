...
#example: gplot_chan_freq.py -data /data/smurf_data/20210219/1613770613/outputs/xxxx.dat -out /xxxx/xxxx/xxx -plot true -chan 333 - band 1
...
for c, (b, ch) in enumerate(zip(bands, channels)):
    if ch < 0:
        continue
    if ch == int(target_chan) and b == int(target_band):
        ch_idx = mask[b, ch]
