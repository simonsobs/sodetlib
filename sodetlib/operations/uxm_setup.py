...
def setup_fixed_tones(
    S, cfg, fixed_tones_per_band=8, min_gap=10, tone_power=10, update_cfg=True
    ):
    """
    ...
    # set the fixed tones
    fixed_tones = {
        b: {"freq_offsets": [], "channels": [], "tone_power": tone_power}
        for b in bands
    }
    for fr in ft_freq:
        ...
        fixed_tones[band]["freq_offsets"].append(foff)
        fixed_tones[band]["channels"].append(channel)

    # update config
    if update_cfg:
        for band, data in fixed_tones.items():
            cfg.dev.update_band(band, {"fixed_tones": data})
