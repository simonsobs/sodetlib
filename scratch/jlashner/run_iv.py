...
if __name__ == '__main__':
    ...
    S.tracking_setup(band, reset_rate_khz=4, lms_freq_hz=20000, fraction_full_scale=frac_pp, make_plot=True,
                     show_plot=False, channel=S.which_on(band), nsamp=2**18, feedback_start_frac=0.02, feedback_end_frac=0.94)
    #tunefile = S.tune_file
    plotdir = S.plot_dir
    iv_file = S.slow_iv_all(band=band, channels=S.which_on(band), bias_groups=np.asarray(
        [bias_group]), overbias_voltage=ob_amp, bias_high=11.0, bias_step=step_size, wait_time=1, high_current_mode=False, overbias_wait=15, cool_wait=300, phase_excursion_min=.1, bias_low=3.5)
    print(iv_file)
    #iv_file = iv_file.replace('_raw_data','')

    with open(args.out_file, 'a') as fname:
        fname.write(
            f'T = {temp} mK, plotdir : {plotdir}, iv_file : {iv_file}\n')
