...
if __name__=='__main__':
    ...
    plotdir = S.plot_dir
    S.slow_iv_all(band = band, channels = S.which_on(band),bias_groups=np.array([1]), overbias_voltage=ob_amp,bias_high=19.9, bias_step=step_size,wait_time=1, high_current_mode=False,overbias_wait=15, cool_wait=30,phase_excursion_min=.0001,bias_low=0) 

    with open(args.out_file, 'a') as fname:
        fname.write(f'T = {temp} mK, plotdir : {plotdir}, tunefile : {tunefile}\n')
