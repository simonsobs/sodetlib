...
print("Taking noise using uc att {} drive {}".format(S.get_att_uc(band), S.amplitude_scale[band]))
...
def rough_tune(current_uc_att, current_tune_power, band):
    ...
    print(
        "lowest rough-tune WL: {} with {} channels out of {}".format(
...
def fine_tune(current_uc_att, current_tune_power, band):
    ...
    wl_median = wl_list[lowest_wl_index]
    print(
        "lowest fine-tune WL: {} with {} channels out of {}".format(
            wl_median, wl_len_list[lowest_wl_index], channel_length
        )
    )
...
# If wl_median actually updated, there's a small chance it could enter here as well. 
...
print("Redoing noise taking at selected uc_atten and drive to get plots at selected attenuation.")
'''S.tracking_setup(
            band,
            reset_rate_khz=cfg.dev.bands[band]["flux_ramp_rate_khz"],
            fraction_full_scale=cfg.dev.bands[band]["frac_pp"],
...
print("last datafile: {}".format(datafile))
